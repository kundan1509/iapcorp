...
/*if(isset($_GET['PaymentId']))
	{
		...
		$query="UPDATE tblpayment SET FinanceVerification='YES' WHERE PaymentId=$PayId AND PaymentType='Advance'";
		...
		{
			echo "<script>alert('Record Not Verified  !')</script>";
		}
...
<body class="nav-md">
	...
	v.ContactNumber,v.emailId,v.ActivationDate,v.VendorStatus,p.PaymentDate
	FROM tblpayment AS p JOIN tblvendorregistration AS v ON p.VendorId=v.VendorId 
	WHERE p.PaymentType='A' AND p.RecStatus='A' AND v.VendorStatus='A' AND FinanceVerification='P' ORDER BY p.PaymentId DESC  LIMIT $start_from,$per_page";				
				...
				if($runQuery)
				{
					while($row=mysqli_fetch_array($runQuery))
					{
						...
						$paymentdates = date("d-m-Y", strtotime($pd));
						...
						echo"<tr>
								...
								<td>$newActDate</td>
								<td>
									<a href='PaymentVerificationDetails.php?PaymentId=$PaymentId' class='btn btn-info'><i class='fa fa-list'></i></a>
								</td>
							</tr>";
					}
			}
...
<?php
            $query = ("SELECT 	p.PaymentId,p.VendorId,p.Amount,p.ModeofPayment,p.TaxnRefNo,p.PaymentDate,p.PaymentProof,p.PaymentType,p.FinanceVerification,
	p.EmailStatus,p.RecStatus,p.LastUpdateOn,p.UpdatebyID,p.DepositBankName,p.VerificationRemark,v.VendorName,v.VendorCode,
	v.ContactNumber,v.emailId,v.ActivationDate,v.VendorStatus 
	FROM tblpayment AS p JOIN tblvendorregistration AS v ON p.VendorId=v.VendorId 
	WHERE p.PaymentType='A' AND p.RecStatus='A' AND v.VendorStatus='A' AND FinanceVerification='P' ORDER BY p.PaymentId DESC");
            $res = mysqli_query($con,$query);
            $total_records = mysqli_num_rows($res); 	
            $total_pages = ceil($total_records / $per_page);
			
              echo "<center> <a href='PaymentVerification.php?page=1' class='btn btn-default'> <i class='fa fa-arrow-left'></i> ".'First'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	          {
              echo "  <a href='PaymentVerification.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
               };
              echo "<a href='PaymentVerification.php?page=$total_pages' class='btn btn-default'>".'Last'." <i class='fa fa-arrow-right'></i></a></center> ";
?>									
