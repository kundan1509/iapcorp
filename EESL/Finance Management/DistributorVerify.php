...
<title>Distributor Verify</title>
<link rel="stylesheet" href="../Designing/dist/css/lightbox.min.css" >
    ...
    <div class="container body">
        <div class="main_container">
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="x_panel">
                                <div class="x_title" style="color:#FFF; background-color:#889DC6;">
                                    <h2>Welcome to The Finance Management Distributor Verification</h2>
          ...
          $start_from = ($page-1) * $per_page;													   
			...
			WHERE v.RecStatus='A' AND v.VendorStatus='P' AND p.PaymentType='S' ORDER BY v.VendorId DESC  LIMIT $start_from,$per_page";
	...
	if($runQuery)
	{
		while($row=mysqli_fetch_array($runQuery))
		{
			...
			echo"<tr>
				...
				<td>$newAppDate</td>
				<td>
					<a href='DistributionDetails.php?DistId=$DistId' class='btn btn-info' title='Record Verification'><i class='fa fa-list'></i></a>
				</td>
			</tr>";
	}
	}
...
<?php
            $query = ("SELECT v.VendorId,v.VendorCode,v.VendorName,v.ContactNumber,v.EmailId,v.VendorStatus,v.ApplicationDate,p.PaymentId,p.PaymentType   
			FROM tblvendorregistration AS v JOIN tblpayment AS p ON  v.VendorId=p.VendorId
			WHERE v.RecStatus='A' AND v.VendorStatus='P' AND p.PaymentType='S' ORDER BY v.VendorId DESC");
            $res = mysqli_query($con,$query);
            $total_records = mysqli_num_rows($res); 	
            $total_pages = ceil($total_records / $per_page);
			
              echo "<center> <a href='DistributorVerify.php?page=1' class='btn btn-default'> <i class='fa fa-arrow-left'></i> ".'First'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	          {
              echo "  <a href='DistributorVerify.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
               };
              echo "<a href='DistributorVerify.php?page=$total_pages' class='btn btn-default'>".'Last'." <i class='fa fa-arrow-right'></i></a></center> ";
?>					 
                  <a href="DistributionDetails.php"	
                  </div>  
                  </div>
                                </div>
                            </div>
