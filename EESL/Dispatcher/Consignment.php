	...
	else
	{
		...
		else
		{
			...
			if(isset($_GET['id']))
			{
				...
				$csid=$_GET['id'];
				$selectverfiy="select AllotmentStatus from tblconsignment where CosigmentId=$csid";
				$selectverfiyQry=mysqli_query($con,$selectverfiy);
				$status=mysqli_fetch_array($selectverfiyQry);
				$stat1=$status['AllotmentStatus'];
				if(($stat1=="P")||($stat1=="R"))
				{
					$deleteCons="update tblconsignment set RecStatus='D',LastUpdateon='$date',UpdatebyId='$RefId' where CosigmentId='$csid'";
					if(mysqli_query($con,$deleteCons))
					{
						echo "<script>alert('Consignment Deleted Successfully')</script>";
						echo "<script>window.open('Consignment.php','_self')</script>";
					}
					else
					{
						echo "<script>alert('Consignment Not Deleted')</script>";
						echo "<script>window.open('Consignment.php','_self')</script>";
					}
				}
				else
				{
					echo "<script>alert('Verified Consignment could not be deleted')</script>";
				}
			}
		}
	}
...
<body class="nav-md">
   <div class="container body">
	...
	$Consignment="SELECT C.CosigmentId,C.ConsignemtNo,W.WareHouseCode,W.WareHouseName,C.RejectedReason,DATE_FORMAT(C.Consignmentdate,'%d-%m-%Y') AS Consignmentdate,C.AllotmentStatus,C.IndentQuantity,C.Quantity,l.locationname
	FROM tblconsignment AS C JOIN tblwarehouseregistration AS W ON C.WarehouseId=W.WarehouseId JOIN tbllocation AS l ON W.warehouselocationid=l.locid WHERE W.RecStatus='A' AND C.RecStatus='A' AND C.WarehouseId='$RefId' AND (C.AllotmentStatus='P' OR C.AllotmentStatus='R')
	ORDER BY C.CosigmentId DESC LIMIT $start_from,$per_page";
	
	//echo $Consignment;//die();
	...
	while($row=mysqli_fetch_array($RunQuery))
	  {
		...
		$Quant=$row['Quantity'];
		$whloc=$row['locationname'];
		$reason=$row['RejectedReason'];
		
		$stat=$row['AllotmentStatus'];
		...
		echo"<tr>
				...
				<a href='editConsignment.php?id=$ConsignmentId' class='btn btn-success' title='Edit Conisgnment Records'><i class='fa fa-edit'></i></a> 
	...
	<?php
        $query = ("SELECT C.CosigmentId,C.ConsignemtNo,W.WareHouseCode,C.RejectedReason,DATE_FORMAT(C.Consignmentdate,'%d-%m-%Y') AS Consignmentdate,C.AllotmentStatus,C.IndentQuantity,C.Quantity,l.locationname
	FROM tblconsignment AS C JOIN tblwarehouseregistration AS W ON C.WarehouseId=W.WarehouseId JOIN tbllocation AS l ON W.warehouselocationid=l.locid WHERE W.RecStatus='A' AND C.RecStatus='A' AND C.WarehouseId='22' AND (C.AllotmentStatus='P' OR C.AllotmentStatus='R') ORDER BY C.CosigmentId DESC");
                $res = mysqli_query($con,$query);
                $total_records = mysqli_num_rows($res); 	
               $total_pages = ceil($total_records / $per_page);	
              echo "<center> <a href='Consignment.php?page=1' class='btn btn-default fa fa-long-arrow-left'>".'First'."</a> ";
