...
if(isset($_SESSION['timeout']))
{
	...
	else
	{
		...
		else
		{
			...
			{
				$RemoveID=$_GET['TempId'];
				$RemoveQuery="UPDATE tbluser SET FinanceVerificationStatus='R',RecStatus='D',LastUpdateOn='',UpdatebyID='$UserId' WHERE Id='$RemoveID' AND RecStatus='A' AND FinanceVerificationStatus='A'";
...
<div class="right_col" role="main">
	<div class="">                    
		...
		$getQuery="SELECT Id,UserId,PASSWORD,UserTypes FROM tbluser WHERE RecStatus='A' AND FinanceVerificationStatus='A' ORDER BY Id DESC  LIMIT $start_from,$per_page";
	...
	if($runQuery)
	{
		while($row=mysqli_fetch_array($runQuery))
		{
			...
			echo"<tr>
				...
				<td>
					<a href='DetailsResetPassword.php?UsrID=$UserID' class='btn btn-info' data-toggle='tooltip' title='Reset Password'><i class='fa fa-list'></i></a>
					<a href='ResetPassword.php?TempId=$UserID' class='btn btn-danger' data-toggle='tooltip' title='Remove Users' onclick=\"return confirm('Are you sure to remove user details ?');\"><i class='fa fa-trash'></i></a>
				</td>
			</tr>";
                  ...
                  <?php
            $query = ("SELECT Id,UserId,PASSWORD,UserTypes FROM tbluser WHERE RecStatus='A' AND FinanceVerificationStatus='A' ORDER BY Id DESC");
            $res = mysqli_query($con,$query);
            $total_records = mysqli_num_rows($res); 	
            $total_pages = ceil($total_records / $per_page);
			
              echo "<center> <a href='ResetPassword.php?page=1' class='btn btn-default'> <i class='fa fa-arrow-left'></i> ".'First'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	          {
              echo "  <a href='ResetPassword.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
               };
              echo "<a href='ResetPassword.php?page=$total_pages' class='btn btn-default'>".'Last'." <i class='fa fa-arrow-right'></i></a></center> ";
?> 
                  </div>  
                  </div>
                                </div>
