...
<?php
include("Function/refdetails.php");
?>
<?php
...
if(isset($_GET['id']))
{
	...
	$refrrid=$_GET['id'];
   $deleteCons="UPDATE tblreferral SET RecStatus='D', UpdateById='$UserId' WHERE ReferralId='$refrrid'";
	...
	while($row=mysqli_fetch_array($RunQuery))
	  {
		...
		echo"<tr>
				...
				<td>$Email</td>
				<td>
				 <a href='DetailsReferral.php?ReferralId=$ReferralId' class='btn btn-success' title='Edit Referral Records'><i class='fa fa-edit'></i></a>
				<a href='refrralList.php?id=$ReferralId' class='btn btn-danger' title='Delete Refrral Records' onclick = 'return confirm('Are you sure want to delete record')'><i class='fa fa-trash'></i></a> 
									...
									<?php
        $query = ("SELECT ReferralId,ReferralType,EmpId,Name,MobileNo,PanNo,Email FROM tblreferral where recStatus='A'
	ORDER BY ReferralId DESC");
                $res = mysqli_query($con,$query);
                $total_records = mysqli_num_rows($res); 	
               $total_pages = ceil($total_records / $per_page);	
              echo "<center> <a href='RefrralList.php?page=1' class='btn btn-default fa fa-long-arrow-left'>".'First'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	          {
              echo "  <a href='RefrralList.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
                };
             echo "<a href=RefrralList.php?page=$total_pages'  class='btn btn-default'>".'Last'." <i class='fa fa-long-arrow-right'></i></a></center> ";
			 ?>		
                              </div>  
                               </div>
                                </div>
