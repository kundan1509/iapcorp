...
<div class="row">
        ...
        <thead>
            <tr>
                <th style="width:250px;">RO E-Mail</th>
                <th style="width:250px;">Administrator E-Mail</th>
        ...
        <tbody>
         ...
         $result = "SELECT * FROM tblMailinglist where RecStatus='A' ORDER BY id DESC LIMIT $start_from,$per_page";
	  $query=mysqli_query($con,$result);
      while ($row = mysqli_fetch_array($query))	  
	  {
      $Ro=$row['RO'];
	  $Admin=$row['SuperAdmin'];
	  $Fin=$row['Finance'];
    ...
    <?php
            $query = ("SELECT * FROM tblMailinglist where RecStatus='A'");
            $res = mysqli_query($con,$query);
            $total_records = mysqli_num_rows($res); 	
            $total_pages = ceil($total_records / $per_page);	
             echo "<center> <a href='EmailList.php?page=1' class='btn btn-default fa fa-long-arrow-left'>".'Previous'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	           {
               echo "  <a href='EmailList.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
               };
               echo "<a href=EmailList.php?page=$total_pages' class='btn btn-default'>".'Next'." <i class='fa fa-long-arrow-right'></i></a></center> ";
?>
