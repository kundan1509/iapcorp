...
if(isset($_POST['SavePoolAccount']))
{
	$BankName=$_POST['BankName'];
	$AccountNo=$_POST['AccountNo'];
	$mergeBankAccount=$BankName.':'.$AccountNo;
	$IfscCode=$_POST['IfscCode'];
	date_default_timezone_set("Asia/Kolkata"); 
    $CurrentDate=date('Y-m-d H:i:s A');
	if($AccountNo!="" && $IfscCode!="")
	{
		$SelectPool="SELECT COUNT(AccountId)AS CountAccount,AccountNo FROM tblpollaccount WHERE RecStatus='A' AND AccountNo='$mergeBankAccount'";
		$runSelectPool=mysqli_query($con,$SelectPool);
		$RowFind=mysqli_fetch_array($runSelectPool);
		if($RowFind['CountAccount']>0)
		{
			echo "<script>alert('Pool Account Already Exist !')</script>";
		}
		else
		{
			$insertPoolAC="INSERT INTO tblpollaccount(BankName,AccountNo,IFSCCode,RecStatus,LastUpdateOn,UpdateById)
	VALUES('$BankName','$AccountNo','$IfscCode','A','$CurrentDate','$UserId')";
			$RinInsert=mysqli_query($con,$insertPoolAC);
			if($RinInsert)
			{
			...
			{
				echo "<script>alert('Record Not Inserted Check Details !')</script>";
			}
...
<div class="row">
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="x_panel">
                                <div class="x_title" style="color:#FFF; background-color:#889DC6">
                                    <h2>Pool Account Configuration</h2>
						...
						<div class="x_content">
                                    ...
                                    <form class="form-horizontal form-label-left" method="post" action="poolbankaccount.php" autocomplete="off" >                
                                        <div class="row">
                                            	<div class="col-lg-2 col-sm-2 col-xs-12">
                                                	<label class="control-label">Bank Name<span style="color:red; margin-top:8px;"> *</span></label>
                                                </div>
                                                <div class="col-lg-4 col-sm-4 col-xs-12">
                                                	<input type="text" name="BankName" maxlength="20" class="textValidate form-control input-sm" required="required" onkeypress="return onlyAlphabets(event,this);" />
                                                 </div>
                                            </div><br />
                                        <div class="row">
                                            	<div class="col-lg-2 col-sm-2 col-xs-12">
                                                	<label class="control-label">Account No.<span style="color:red; margin-top:8px;"> *</span></label>
                                                </div>
                                                <div class="col-lg-4 col-sm-4 col-xs-12">                                                	
                                                    <input type="text" name="AccountNo" onpaste="return false" maxlength="20" class="txtNumeric form-control input-sm" required="required" />
                                                </div>
                                            </div><br />
                                        <div class="row">
                                            	<div class="col-lg-2 col-sm-2 col-xs-12">
                                                	<label class="control-label">IFSC Code<span style="color:red; margin-top:8px;"> *</span></label>
                                                </div>
                                                <div class="col-lg-4 col-sm-4 col-xs-12">
                                                	<input type="text" name="IfscCode" onpaste="return false" maxlength="20" class="form-control input-sm" required="required"/>
                                                </div>
                                            </div><br />                               
                                        <div class="form-group">
                                            <div class="col-md-6 col-md-offset-3">                                               
                                                <button type="submit" name="SavePoolAccount" class="btn btn-primary">Save Record</button>
                                            </div>
                                        </div>
