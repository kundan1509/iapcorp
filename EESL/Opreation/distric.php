...
if(isset($_POST['SaveDistric']))
{	
	$SelectState=$_POST['SelectState'];
	$DistricName=$_POST['DestricName'];
	$DistricCode=$_POST['DistricCode'];
	date_default_timezone_set("Asia/Kolkata"); 
    $CurrentDate=date('Y-m-d H:i:s A');
	if($SelectState!='0')
	{
		if($DistricName!="" && $DistricCode!="")
		{
			$SelectDistric="SELECT COUNT(CityId) AS CityCount,CityName,CityCode FROM tblcity WHERE RecStatus='A' AND CityName='$DistricName'";
			$RunDistric=mysqli_query($con,$SelectDistric);
			$RowDistric=mysqli_fetch_array($RunDistric);
			if($RowDistric['CityCount']<1)
			{
				
				$InsertDistric="INSERT INTO tblcity(CityName,CityCode,StateId,LastUnitVendor,LastUnitWarehouse,LastUpdateOn,UpdateById,RecStatus)
	                            VALUES('$DistricName','$DistricCode','$SelectState','0','0','$CurrentDate','$UserId','A')";
				$RunInsert=mysqli_query($con,$InsertDistric);
				if($RunInsert)
				{
					echo "<script>alert('District Inserted Successfully !')</script>";
						echo "<script>window.open(distric.php','_self')</script>";
				}
				else
				{
					echo "<script>alert('District Name is not Inserted Try Again !')</script>";
						echo "<script>window.open(distric.php','_self')</script>";
				}
			}
			else
...
?>
<?php
	$SelectState="SELECT Stateid,StateName,StateCode FROM tblstate WHERE RecStatus='A'";
                    ...
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="x_panel" style="height:500px;">
                                ...
                                <div class="x_content">
                                	<form method="post" action="distric.php">
                                    	<div class="col-lg-12 col-sm-12 col-xs-12">
                                        	<div class="row">
                                            	<div class="col-lg-2 col-sm-2 col-xs-12">
                                                	<label class="control-label">Select State<span style="color:red; margin-top:8px;"> *</span></label>
                                                </div>
                                                <div class="col-lg-4 col-sm-4 col-xs-12">
                                                	<select name="SelectState" required="required" class="form-control input-sm">
                                                    	<option value="0">Select State</option>
                                                        <?php
															while($RowCount=mysqli_fetch_array($SelectQuery))
															{
																$StateId=$RowCount['Stateid'];
																$StateName=$RowCount['StateName'];
                                            ...
                                            <div class="row">
                                            	<div class="col-lg-2 col-sm-2 col-xs-12">
                                                	<label class="control-label">District Name<span style="color:red; margin-top:8px;"> *</span></label>
                                                </div>
                                                <div class="col-lg-4 col-sm-4 col-xs-12">
                                                	<input type="text" name="DestricName" maxlength="20" class="textValidate form-control input-sm" required="required" onkeypress="return onlyAlphabets(event,this);" />
                                                </div>
                                            </div><br />
                                            <div class="row">
                                            	<div class="col-lg-2 col-sm-2 col-xs-12">
                                                	<label class="control-label">District Code<span style="color:red; margin-top:8px;"> *</span></label>
                                                </div>
                                                <div class="col-lg-4 col-sm-4 col-xs-12">
                                                	<input type="text" name="DistricCode" maxlength="5" class="textValidate form-control input-sm" required="required" onkeypress="return onlyAlphabets(event,this);" />
                                                </div>
                                            </div><br />
                                            
                                            <div class="row">
                                            	<div class="col-lg-3 col-sm-3 col-xs-12 col-sm-offset-2">
                                                	<input type="submit" name="SaveDistric" class="btn btn-primary" value="Save Record" />
                                                </div>
                                            </div>
                                        </div>
                                    </form>
