...
<?php
	...
	{
		$CityId=$_GET['CityId'];
		$RemoveQuery="UPDATE tblcity SET RecStatus='D' WHERE CityId='$CityId' AND RecStatus='A' ";
		$RemoveRun=mysqli_query($con,$RemoveQuery);
		if($RemoveRun)
		{
			echo "<script>alert('District Removed Successfully !')</script>";			
		}
		else
		{
		
			echo "<script>alert('District not removed !')</script>";
		}
	}
?>
...
<div class="row">
        ...
        <thead>
            <tr>
                ...
                <th style="width:250px;">District Name</th> 
                 <th style="width:250px;">District  Code</th>         		
	    ...
	    $result= "SELECT S.StateName,VD.CityName,VD.CityCode,VD.CityId FROM tblstate AS S JOIN  tblcity AS VD      
                   WHERE  S.Stateid=VD.StateId AND  VD.RecStatus='A'  ORDER BY VD.CityId DESC LIMIT $start_from,$per_page";
	    $query=mysqli_query($con,$result);
        while ($row = mysqli_fetch_array($query))	  
	    {
		$CityId=$row['CityId'];     
	    $statename=$row['StateName'];
        $districtname=$row['CityName'];
		$districtcode=$row['CityCode'];
		 ...
		 echo "
            <tr>
                ...
                <td>
				     ...
				     <a href='DetailsDistrict.php?CityId=$CityId' class='btn btn-success' title='Edit District Records'><i class='fa fa-edit'></i></a>
                    <a href='DistrictList.php?CityId=$CityId' class='btn btn-danger' data-toggle='tooltip' title='Remove District' onclick=\"return confirm('Are You sure Remove User Details ?');\"><i class='fa fa-trash'></i></a>					 
    ...
    <?php
            $query = ("SELECT S.StateName,VD.CityName,VD.CityCode,VD.CityId FROM tblstate AS S JOIN  tblcity AS VD      
                   WHERE  S.Stateid=VD.StateId AND  VD.RecStatus='A'  ORDER BY VD.CityId DESC");
            $res = mysqli_query($con,$query);
            $total_records = mysqli_num_rows($res); 	
            $total_pages = ceil($total_records / $per_page);	
             echo "<center><a href='DistrictList.php?page=1' class='btn btn-default fa fa-long-arrow-left'>".'First'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	           {
               echo "  <a href='DistrictList.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
               };
               echo "<a href=DistrictList.php?page=$total_pages' class='btn btn-default'>".'Last'." <i class='fa fa-long-arrow-right'></i></a></center> ";
?>
