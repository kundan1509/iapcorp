...
<?php
	if(isset($_GET['Stateid']))
	{
		...
		$RemoveQuery="UPDATE tblstate SET RecStatus='D' WHERE Stateid='$Stateid' AND RecStatus='A' ";
...
<div class="row">
        ...
        <thead>
            <tr>
                <th style="width:250px;">State Code </th>
                <th style="width:250px;">State Name </th>      
                 <th style="width:250px;">Buy Rate</th> 
                  <th style="width:250px;">Sell Rate</th>     		
       ...
       $result = "SELECT Stateid,StateCode,StateName,buyrate,sellrate FROM tblstate where RecStatus='A' ORDER BY Stateid DESC LIMIT $start_from,$per_page";
		 ...
		 echo "
            <tr>
                ...
                <td>
				     ...
				     <a href='UpdateState.php?Stateid=$Stateid' class='btn btn-success' title='Edit State Records'><i class='fa fa-edit'></i></a>
                    <a href='StateList.php?Stateid=$Stateid' class='btn btn-danger' data-toggle='tooltip' title='Remove State' onclick=\"return confirm('Are You sure Remove User Details ?');\"><i class='fa fa-trash'></i></a>					 
    ...
    <?php
            $query = ("SELECT Stateid,StateCode,StateName,buyrate,sellrate FROM tblstate where RecStatus='A' ORDER BY Stateid DESC");
            $res = mysqli_query($con,$query);
            $total_records = mysqli_num_rows($res); 	
            $total_pages = ceil($total_records / $per_page);	
             echo "<center><a href='StateList.php?page=1' class='btn btn-default fa fa-long-arrow-left'>".'Previous'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	           {
               echo "  <a href='StateList.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
               };
               echo "<a href=StateList.php?page=$total_pages' class='btn btn-default'>".'Next'." <i class='fa fa-long-arrow-right'></i></a></center> ";
?>
