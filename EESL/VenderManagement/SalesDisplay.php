...
<body class="nav-md">
    <div class="container body">
       ...
       <table class="table table-striped table-bordered">
        <thead>
            <tr>
                ...
                <th style="width:250px;">Distribution Date</th>			
      ...
      <?php
		  ...
		  $result= "SELECT L.LocationName,VS.Quantity,DATE_FORMAT(VS.SaleDate,'%d-%m-%y') AS SaleDate,VS.SalesId FROM tbllocation AS L JOIN tblvendorlocation AS VLOC JOIN vendorsales AS VS 		     
            WHERE L.LocId=VLOC.LocationId AND VLOC.locationid=VS.LocationId AND  VS.RecStatus='A'  AND VS.Vendorid='$Refid' ORDER BY VS.SalesId DESC  LIMIT $start_from,$per_page";
	    ...
	    $saledate=$row['SaleDate'];
		 ...
		 echo "
            <tr>
				 ...
				 <td> $saledate</td>     				 
                <td>
				     <a href='DetailsDistribution.php?SalesId=$SalesId' class='btn btn-info' title='View Distribution Records'><i class='fa fa-list'></i></a>         
                </td>
            </tr>";
	  }
			?>  		
...
<?php
            $query = ("SELECT L.LocationName,VS.Quantity,DATE_FORMAT(VS.SaleDate,'%d-%m-%y') AS SaleDate,VS.SalesId FROM tbllocation AS L JOIN tblvendorlocation AS VLOC JOIN vendorsales AS VS 		     
            WHERE L.LocId=VLOC.LocationId AND VLOC.locationid=VS.LocationId AND  VS.RecStatus='A'  AND VS.Vendorid='$Refid' ORDER BY VS.SalesId DESC");
            $res = mysqli_query($con,$query);
            $total_records = mysqli_num_rows($res); 	
            $total_pages = ceil($total_records / $per_page);	
             echo "<center> <a href='SalesDisplay.php?page=1' class='btn btn-default fa fa-long-arrow-left'>".'First'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	         {
             echo "  <a href='SalesDisplay.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
             };
             echo "<a href=SalesDisplay.php?page=$total_pages' class='btn btn-default'>".'Last'." <i class='fa fa-long-arrow-right'></i></a></center> ";
?>    
    </div>
         </div>
               </div>
