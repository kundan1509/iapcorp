...
<div  class="table-responsive">                                        
       <table  class="table table-striped table-bordered  ">
        <thead>
            <tr>            
                ...
                <th style="width:250px;">Mode Of Payment</th>
                <th style="width:200px;"> Txn Reference No</th>
                <th style="width:200px;">Payment Date</th>
                <th style="width:200px;">Payment Proof</th>			
                 <th style="width:200px;">Bank Name</th>
                 <th style="width:250px;">Verification Remarks</th>				 
  ...
  <?php 
          ...
          $start_from = ($page-1) * $per_page;	
             $getpayment="SELECT p.PaymentId, p.Amount,p.ModeofPayment,p.TaxnRefNo,p.PaymentDate,p.PaymentProof,p.DepositBankName,p.VerificationRemark FROM tblpayment 
					AS p WHERE p.RecStatus='A' AND VendorId=$Refid AND p.FinanceVerification<>'Y' and p.PaymentType='A' ORDER BY p.PaymentId DESC  LIMIT $start_from,$per_page"; 
						 
	  $query=mysqli_query( $con,$getpayment);	  
	  ...
	  $txnno=$row['TaxnRefNo'];
	  $PayDate1=$row['PaymentDate'];
	  $PayDate = date("d-m-Y", strtotime($PayDate1));
	  $proof=$row['PaymentProof'];	
	  $bankname=$row['DepositBankName'];
      $VerificationRemark=$row['VerificationRemark'];
		 ...
		 echo "
            <tr>
				 ...
				 <td> $txnno</td>
				 <td>$PayDate</td>	
                  <td><a href='$proof'  class='example-image-link' data-lightbox='example-1'><img src='$proof' alt='' class='img-responsive' style='max-height:50px; max-width:100px;'></a></td> 
                 <td> $bankname</td>
                 <td> $VerificationRemark</td>  				  
                <td>

					<a href='UpdatePayments.php?PaymentId=$PaymentId' '_self' class='btn btn-success' title='Edit Payment Records'><i class='fa fa-edit'></i></a>
				     <a href='ViewPayment.php?PaymentId=$PaymentId' class='btn btn-info' title='View Payment Records'><i class='fa fa-list'></i></a>    
                </td>
            </tr>";
...
<?php
         $query = ("SELECT p.PaymentId, p.Amount,p.ModeofPayment,p.TaxnRefNo,DATE_FORMAT(p.PaymentDate,'%d-%m-%y') AS
	                PaymentDate,p.PaymentProof,p.DepositBankName,p.VerificationRemark FROM tblpayment 
					AS p WHERE p.RecStatus='A' AND VendorId=$Refid AND p.FinanceVerification<>'Y' and p.PaymentType='A' ORDER BY p.PaymentId DESC");
             $res = mysqli_query($con,$query);
            $total_records = mysqli_num_rows($res); 	
             $total_pages = ceil($total_records / $per_page);
		  
              echo "<center> <a href='PaymentDisplay.php?page=1' class='btn btn-default fa fa-long-arrow-left'>".'First'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	          {
              echo "  <a href='PaymentDisplay.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
              };
             echo "<a href=PaymentDisplay.php?page=$total_pages' class='btn btn-default'>".'Last'." <i class='fa fa-long-arrow-right'></i></a></center> ";
?>
	       </div>
               </div>
                  </div>
