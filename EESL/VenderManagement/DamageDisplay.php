...
<body class="nav-md">
       ...
       <table class="table table-striped table-bordered">
        <thead>
            <tr>
                ...
                <th style="width:400px;">Damage Stock Date</th>
 ...
 <?php		  
		  ...
		  $result= "SELECT w.warehousename,w.warehousecode,L.LocationName,VD.Quatity,VD.DamageDate,VD.Reason,VD.VerifyRemarks,VD.DamageId FROM tbllocation AS L JOIN tblvendorlocation AS VL JOIN vendordamagestock AS VD JOIN tblwarehouseregistration AS w   
 WHERE L.LocId=VL.LocationId AND w.warehouseid=vd.warehouseid AND VL.locationid=VD.LocationId AND  VD.RecStatus='A' AND VD.DamageStatus='N' AND VD.Vendorid= '$Refid' ORDER BY VD.DamageId DESC LIMIT $start_from,$per_page";
		...
		$whname=$row['warehousename'];
		 echo "
            <tr>
                ...
                <td>
                      <a href='UpdateDamageStock.php?DamageId=$DamageId' class='btn btn-success' title='Edit Damage Stock Records'><i class='fa fa-edit'></i></a>
				     <a href='ViewDamage.php?DamageId=$DamageId' class='btn btn-info' title='View Damage Stock Records'><i class='fa fa-list'></i></a>			 
	...
	<?php
            $query = ("SELECT L.LocationName,VD.Quatity,VD.DamageDate,VD.Reason,VD.VerifyRemarks,VD.DamageId FROM tbllocation AS L JOIN tblvendorlocation AS VL JOIN vendordamagestock AS VD      
                        WHERE L.LocId=VL.LocationId AND VL.locationid=VD.LocationId AND  VD.RecStatus='A' AND VD.DamageStatus='N' AND VD.Vendorid= '$Refid'  ORDER BY VD.DamageId DESC");
            $res = mysqli_query($con,$query);
            $total_records = mysqli_num_rows($res); 	
            $total_pages = ceil($total_records / $per_page);	
             echo "<center> <a href='DamageDisplay.php?page=1' class='btn btn-default fa fa-long-arrow-left'>".'First'."</a> ";
              for ($i=1; $i<=$total_pages; $i++)
	           {
               echo "  <a href='DamageDisplay.php?page=".$i."' class='btn btn-default'>".$i."</a> ";
               };
               echo "<a href=DamageDisplay.php?page=$total_pages' class='btn btn-default'>".'Last'." <i class='fa fa-long-arrow-right'></i></a></center> ";
?>
